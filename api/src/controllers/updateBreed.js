...
const updateBreed = async (req,res) => {
  ...
  const {name,image,height,weight,age,temperament,userId,prevImage} = req.body;

  // Breed id
  const {id} = req.params
  
  let prevImgSplit = prevImage.split('/');
  let prevImg = prevImgSplit[prevImgSplit.length - 1];
  ...
  try {
    ...
    if(image) {
      rimraf(`C:/Users/user/Downloads/PI-Dogs-main/api/src/images/${prevImg}`);
    }

    /* if(image !== null)  {
      try {
        fs.unlinkSync(`C:/Users/user/Downloads/PI-Dogs-main/api/src/images/${prevImg}`)
        //file removed
      } catch(err) {
        console.error(err);
      }
